...
DWORD load_header (void)	/* 0:Invalid format, 1:I/O error, >1:Number of samples */
{
	...
	lcd_text("WAVE File found:", 5, 115, LCD_RED, LCD_GREEN);
	...
	while(1) {
		...
		switch (ChunkID) {	/* FCC */
			case fmt_chunk:					/* 'fmt ' chunk */
				lcd_text("fmt chunk found", 5, 85, LCD_RED, LCD_GREEN);
				...
				if (NumChannels != 1 && NumChannels != 2) 		/* Check channels (1/2) */
					return 0;				
				if( NumChannels == 1)	lcd_text("Mono", 10, 70, LCD_RED, LCD_GREEN);
				else if( NumChannels == 2)	lcd_text("Stereo", 10, 70, LCD_RED, LCD_GREEN);

				BitsPerSample = Buff[14];		/* Resolution flag */
				if (BitsPerSample != 8 && BitsPerSample != 16)	/* Check resolution (8/16) */
					return 0;				
				if( BitsPerSample == 8)	lcd_text("8-bits", 55, 70, LCD_RED, LCD_GREEN);
				else if( BitsPerSample == 16)	lcd_text("16-bits", 55, 70, LCD_RED, LCD_GREEN);

				SamplingRate = LD_DWORD(Buff+4);
				set_sampling_rate( SamplingRate );			/* Sampling freq */
				sprintf(str, "%ldHz Sampling", SamplingRate);
				//lcd_text(str, 10, 60, LCD_RED, LCD_GREEN);
				lcd_text2(str, 10, 60, LCD_RED, LCD_GREEN);
				break;
	
			case data_chunk:				/* 'data' chunk (start to play) */
				lcd_text("data chunk found", 5, 40, LCD_RED, LCD_GREEN);
				return sz;
	
			case LIST_chunk:				/* 'LIST' chunk (skip) */
				lcd_text("LIST chunk (skip)", 5, 40, LCD_RED, LCD_GREEN);
			case fact_chunk:				/* 'fact' chunk (skip) */
				pf_lseek(Fs.fptr + sz);
				lcd_text("fact chunk (skip)", 5, 40, LCD_RED, LCD_GREEN);
				break;
	
			default :								/* Unknown chunk (error) */
				lcd_text("Unknown chunk (error)", 5, 40, LCD_RED, LCD_GREEN);
				return 0;
		}
	}
	return 0;
...
UINT play_wave_file(const char *fn)
{
	...
	if ((res = pf_open(fn)) == FR_OK) {
		...
		lcd_text(fn, 15, 100, LCD_RED, LCD_GREEN);
		if (sz < 256) return (UINT)sz;
		lcd_text("file size ok", 10, 20, LCD_RED, LCD_GREEN);
		enable_audio();
		if( (NumChannels==2) && (BitsPerSample==16) ){
			...
			lcd_text("format error", 10, 30, LCD_RED, LCD_GREEN);
		}
		disable_audio();
